...
class FileIconMapping(TypedDict):
    icon: str
    color: str


FILE_ICONS: Final[dict[str, FileIconMapping]] = {
...
def get_path_icon(path: str | os.PathLike[str]) -> str:
    ...
    if path_obj.name.lower() in FILE_ICONS:
        return FILE_ICONS[path_obj.name.lower()]["icon"]
